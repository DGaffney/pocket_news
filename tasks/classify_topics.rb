class ClassifyTopics
  ...
  def get_tags(article, item_id, url)
    if (tag = ArticleTag.first(item_id: item_id, url: url)).nil?
      JSON.parse(ArticleTag.from_raw(JSON.parse(RestClient.post("https://api.meaningcloud.com/topics-1.2", {key: SETTINGS["meaningcloud"], lang: "en", txt: article.get(:content_stripped), txtf: "plain", tt: "a"})), item_id, url).to_json)
    else
